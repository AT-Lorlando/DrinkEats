...
class App extends React.Component {
  ...
  addToCart = (id, quantity) => {
    ...
    if (this.state.cart.items.find(item => item.id === soup.id)) {
      ...
      this.setState((state) => ({
        cart: {
          q: state.cart.q + quantity, // utilise des noms lisibles pour tes variables pas q ou t
          t: state.cart.t + (soup.price * quantity),
  ...
  componentDidMount() {
    ...
    axios.get(`${url}/api/soup/`, {
      headers: {
        ...
        // 'Access-Control-Allow-Origin': '*', // à supprimer, inutile côté client
      ...
      .then((res) => {
        ...
        res.data.soups.forEach(s => {
          fetchedSoups.push(new soup(s.title, s._id, s.description, s.details, s.price, s.quantity)); // les variables encore "s"
        });
    ...
    if (param.email && param.password) {
      ...
      axios.post(`${url}/api/auth/login`, {
        ...
        .then((res) => {
          ...
          if (res.data.token) {
            this.setState({ // il ne faut pas les mettre dans le state, si on recharge la page on perd le login, il faut mettre dans un cookie ou dans le localStorage
  ...
  // ce fichier est très long tu aurais pû diviser en plusieurs sous modules
  sendCommand = (command) => {
    ...
    axios.post(`${url}/api/order/`, {
      ...
      headers: {
        Authorization: `Bearer ${this.state.token}`, // top
        "Content-Type": "application/json; charset=utf-8",
        'Accept': 'application/json'
      }
    })
